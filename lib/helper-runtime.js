...
export function createHelperRuntime({ id = randomUUID(), cardId = '', rawCard = null, scripts = [], variables = {}, tools = [], messages = [], sessionId = '', chatId = '', cardName = '', userName = '', extensionSettings = {}, worldbook = {} } = {}) {
  ...
  const queue = []
  // N5: 回滚栈（消息+变量快照，最多 50 层）。
  const rollbackStack = []
  function pushRollbackSnapshot(runtime, label) {
    rollbackStack.push({ label: String(label || ''), at: Date.now(), messages: clone(runtime.messages) || [], variables: clone(runtime.variables) || {} })
    if (rollbackStack.length > 50) rollbackStack.splice(0, rollbackStack.length - 50)
  }
  const runtime = {
    ...
    // N5: regen —— 删除最后一条 assistant 消息，返回可用于重新生成的上下文。
    // 对标上游 regenBody（去掉最后一条 AI 回复，重跑生成）。
    regenLast() {
      const list = this.messages || []
      for (let i = list.length - 1; i >= 0; i--) {
        if (String(list[i]?.role || '') === 'assistant') {
          const [removed] = list.splice(i, 1)
          pushRollbackSnapshot(this, 'regen')
          this.updatedAt = Date.now()
          recordAudit(this, { type: 'regen', messageId: String(removed?.id || '') })
          this.messageDeleted({ messageId: String(removed?.id || '') }).catch(() => {})
          return { removed: clone(removed), messages: clone(list) }
        }
      }
      return { removed: null, messages: clone(list) }
    },
    // N5: rollback —— 回退到第 index 条消息之后（含 index），变量同步快照恢复。
    rollbackTo(index, { label = '' } = {}) {
      const list = this.messages || []
      const at = Math.max(-1, Math.min(list.length - 1, Number(index)))
      pushRollbackSnapshot(this, label || `rollbackTo:${at}`)
      const kept = at < 0 ? [] : list.slice(0, at + 1)
      const removed = list.slice(kept.length)
      this.messages = kept
      // 变量回退：如果栈中有早于此长度的快照则恢复，否则保持
      const snap = rollbackStack.length > 1 ? rollbackStack[rollbackStack.length - 2] : null
      if (snap && Array.isArray(snap.messages) && snap.messages.length <= kept.length) {
        this.variables = clone(snap.variables) || {}
      }
      this.updatedAt = Date.now()
      recordAudit(this, { type: 'rollback', at, removed: removed.length })
      return { at, kept: clone(kept), removed: clone(removed) }
    },
    // N5: undo rollback —— 恢复最近一次 rollback 前的快照。
    undoRollback() {
      const snap = rollbackStack.pop()
      if (!snap) return null
      this.messages = clone(snap.messages) || []
      this.variables = clone(snap.variables) || {}
      this.updatedAt = Date.now()
      recordAudit(this, { type: 'undo-rollback', label: snap.label })
      return { label: snap.label, messages: clone(this.messages), variables: clone(this.variables) }
    },
    rollbackDepth() { return rollbackStack.length },
    // N5: candidates —— 为当前上下文生成 N 个候选槽位（文本由调用方经 native/turn 填充）。
    // 对标上游 candidate-generation/tasks：此处只管槽位与选中语义，不调模型。
    createCandidates(count = 3, { label = '' } = {}) {
      const n = Math.max(1, Math.min(10, Number(count) || 3))
      const batchId = randomUUID()
      const items = []
      for (let i = 0; i < n; i++) {
        items.push({ id: randomUUID(), batchId, index: i, label: String(label || ''), text: '', selected: false, createdAt: Date.now() })
      }
      if (!Array.isArray(this.candidateBatches)) this.candidateBatches = []
      this.candidateBatches.push({ batchId, label: String(label || ''), items, createdAt: Date.now() })
      if (this.candidateBatches.length > 20) this.candidateBatches = this.candidateBatches.slice(-20)
      recordAudit(this, { type: 'candidates-create', batchId, count: n })
      return clone({ batchId, items })
    },
    fillCandidate(batchId, candidateId, text) {
      const batches = Array.isArray(this.candidateBatches) ? this.candidateBatches : []
      const batch = batches.find(item => String(item?.batchId || '') === String(batchId || ''))
      if (!batch) return null
      const candidate = batch.items.find(item => String(item?.id || '') === String(candidateId || ''))
      if (!candidate) return null
      candidate.text = String(text ?? '')
      this.updatedAt = Date.now()
      return clone(candidate)
    },
    selectCandidate(batchId, candidateId) {
      const batches = Array.isArray(this.candidateBatches) ? this.candidateBatches : []
      const batch = batches.find(item => String(item?.batchId || '') === String(batchId || ''))
      if (!batch) return null
      let picked = null
      for (const item of batch.items) {
        item.selected = String(item?.id || '') === String(candidateId || '')
        if (item.selected) picked = item
      }
      if (!picked) return null
      // 选中即落为 assistant 消息
      const record = this.createMessage({ role: 'assistant', content: picked.text })
      recordAudit(this, { type: 'candidates-select', batchId, candidateId })
      return { candidate: clone(picked), message: clone(record) }
    },
    // N5: branch —— 从当前状态分叉出新 runtime（消息+变量深拷贝，会话隔离）。
    branch(newId) {
      const fork = createHelperRuntime({
        id: newId || undefined,
        cardId: this.cardId, rawCard: null, scripts: clone(this.scripts),
        variables: clone(this.variables), messages: clone(this.messages),
        sessionId: this.sessionId, chatId: this.chatId,
        cardName: this.cardName, userName: this.userName,
        extensionSettings: clone(this.extensionSettings), worldbook: clone(this.worldbook)
      })
      recordAudit(this, { type: 'branch', from: this.id, to: fork.id })
      return fork
    },
