...
async function tavernRpcRequest(dshHome, method, args = {}) {
  ...
  try { payload = JSON.parse(text) } catch {
    throw new Error('Tavern RPC 返回不是 JSON: ' + text.slice(0, 300))
  }
  if (!response.ok || payload.ok === false) throw new Error(payload.error || ('Tavern RPC HTTP ' + response.status))
  return payload
}
...
function buildMuseSystem(context, mode) {
  const lines = []
  if (mode === 'adventure') {
  ...
  if (context.worldbook && context.worldbook.entries.length > 0) {
    ...
    for (const entry of context.worldbook.entries.filter(entry => entry.enabled).slice(0, 24)) {
      ...
      handler: async (req, res) => {
        ...
        try {
          ...
          if (req.method === 'GET' && pathname === '/plugins/creative-suite/tavern/gameplay/capabilities') {
            const result = await tavernGameplayRequest(dshHome, 'capabilities')
