...
function extractRegexScriptsFromPreset(record) {
  ...
  // raw 为纯正则数组（顶层 list 导入）：直接取用。
  const rawScripts = Array.isArray(raw) && raw.length > 0 && raw.every(item => item && typeof item === 'object' && ('scriptName' in item || 'findRegex' in item))
    ? raw
    : null
  ...
  const binding = extensions.SPreset && extensions.SPreset.RegexBinding && Array.isArray(extensions.SPreset.RegexBinding.regexes)
    ...
    : []
  const scripts = rawScripts || (Array.isArray(extensions.regex_scripts) && extensions.regex_scripts.length > 0
...
function unwrapPresetInput(value, fallbackName) {
  let root = value && typeof value === 'object' && !Array.isArray(value) ? value : {}
  if (root.kind === 'dsh-tavern-character-workspace' && root.raw && typeof root.raw === 'object') root = root.raw.data || root.raw
  // 纯正则数组文件（顶层 list，每项含 scriptName/findRegex）：直接视为 regex_scripts。
  if (Array.isArray(value) && value.length > 0 && value.every(item => item && typeof item === 'object' && ('scriptName' in item || 'findRegex' in item))) {
    root = { name: fallbackName, extensions: { regex_scripts: value } }
  }
...
// PNG 角色卡解析：SillyTavern 约定，卡数据以 base64 存于 tEXt chunk（keyword 'chara'）。
// dataUrl 或纯 base64 -> inflate -> JSON。抛错由调用方转 400。
function extractCardFromPngDataUrl(dataUrl) {
  const m = /^data:image\/png;base64,([\s\S]+)$/.exec(String(dataUrl || '').trim())
  const b64 = m ? m[1] : String(dataUrl || '').trim()
  let buf
  try { buf = Buffer.from(b64, 'base64') } catch { throw new Error('PNG base64 无效') }
  ...
  while (offset + 8 <= buf.length) {
    ...
    if (type === 'tEXt') {
      ...
      if (nul > dataStart && nul < dataEnd) {
        const keyword = buf.toString('latin1', dataStart, nul)
        if (keyword === 'chara') {
          const text = buf.toString('latin1', nul + 1, dataEnd)
          try {
            const json = Buffer.from(text, 'base64').toString('utf-8')
            return { card: JSON.parse(json), imageDataUrl: m ? String(dataUrl).slice(0, 30) + '...' : undefined }
          } catch { throw new Error('PNG 内嵌角色卡数据损坏') }
        }
      }
    } else if (type === 'iTXt') {
      ...
      if (nul > p && nul < dataEnd) {
        const keyword = buf.toString('latin1', p, nul)
        p = nul + 1
        if (p + 2 <= dataEnd && keyword === 'chara') {
          const compFlag = buf[p]; const compMethod = buf[p + 1]; p += 2
          const langEnd = buf.indexOf(0, p); if (langEnd < 0 || langEnd >= dataEnd) { offset = dataEnd + 4; continue }
          p = langEnd + 1
          const transEnd = buf.indexOf(0, p); if (transEnd < 0 || transEnd >= dataEnd) { offset = dataEnd + 4; continue }
          p = transEnd + 1
          try {
            let json
            const raw = buf.subarray(p, dataEnd)
            if (compFlag === 1) {
              if (compMethod !== 0) throw new Error('不支持的压缩方法')
              json = inflateSync(raw).toString('utf-8')
            } else {
  ...
  throw new Error('PNG 中未找到内嵌角色卡（tEXt chara）')
}
...
function buildMuseContext(store, body) {
  ...
  const worldbook = worldbookRecord
    ...
    : null
  // N7: preset 接入 gameplay —— prompts 拼 system，regex 出站过滤。
  const presetId = input.presetId ? String(input.presetId) : ''
  const presetRecord = presetId ? store.data.resources.presets[presetId] : null
  const presetPromptLines = presetRecord ? presetPromptsForSystem(presetRecord) : []
  return { card, companions, companionIds: companions.map(item => item.id), worldbook, presetId: presetRecord ? presetRecord.id : '', presetName: presetRecord ? presetRecord.name : '', presetPromptLines, presetRecord }
}
...
// preset prompts 拼 system：从 raw.prompts 取 enabled + content（marker/system 按序），ST marker 不渲染内容。
function presetPromptsForSystem(preset) {
  const raw = preset && preset.data && preset.data.raw
  const prompts = raw && Array.isArray(raw.prompts) ? raw.prompts : []
  const orderGroup = raw && Array.isArray(raw.prompt_order)
    ? (raw.prompt_order.find(g => g && Number(g.character_id) === 100001 && Array.isArray(g.order))
      || raw.prompt_order.find(g => g && Array.isArray(g.order)))
    : null
  const order = orderGroup && Array.isArray(orderGroup.order) ? orderGroup.order.map(String) : []
  const byId = new Map()
  ...
  for (const p of ordered) {
    if (!p || p.enabled === false || p.marker === true) continue
    const content = String(p.content || '').trim()
    if (!content) continue
    const role = String(p.role || 'system')
    lines.push(`[${p.name || p.identifier || role}] ${content.slice(0, 2000)}`)
  }
  return lines
}
...
function buildMuseSystem(context, mode, opts = {}) {
  ...
  if (mode === 'adventure') {
    ...
    lines.push('你是 DSH Creative Suite 的 Tavern 角色扮演引擎。当前为酒馆模式：全程只以 ST 人物卡指定的这一个角色身份与玩家对话，绝不切换成旁白 GM、绝不扮演其他 NPC（包括老人/店小二/同伴），也绝不自称“老者/老朽/在下以外的他人”。保持性格、性别、场景与关系一致，沉浸式演出。')
  } else {
  ...
  lines.push('')
  if (context.presetPromptLines && context.presetPromptLines.length > 0) {
    lines.push(`预设指令（${context.presetName || '预设'}，按顺序生效）：`)
    for (const line of context.presetPromptLines.slice(0, 60)) lines.push(line)
    lines.push('')
  }
  if (mode === 'adventure') {
    ...
    lines.push('输出要求：只输出本角色的第一人称对白与动作（可用“她/书瑶”自指但身份不变），不要解释系统提示，不要输出 Markdown 代码块。中文默认，保持沉浸感。')
  }
  return lines.join('\n')
}
...
function createMuseSession(store, body) {
  const input = body && typeof body === 'object' ? body : {}
  const now = Date.now()
  const mode = input.mode === 'adventure' ? 'adventure' : (input.mode === 'booktravel' ? 'booktravel' : 'chat')
  const cardId = input.cardId ? String(input.cardId) : ''
...
// N8 穿书 prompt（移植自 MuseAI 原版 useSettingsStore.ts 默认值，精简为核心约束）。
const BOOKTRAVEL_ASSEMBLER_PROMPT = `你是穿书素材装配师。你负责读取用户选中的大纲、世界书和角色卡，把它们整理成可运行的穿书世界模型。

## 输出要求
- 只输出 JSON，不要输出 Markdown 代码块。
- 保留原始时间线、核心冲突、可能结局、世界规则、重要地点、势力关系、角色画像、关系暗线和隐藏信息边界。
- 只能整理和归纳素材，不得改写、覆盖或再生成世界书与角色卡。
- 所有 JSON 字符串值中不得包含未转义的 ASCII 双引号。如需引述，请使用中文引号「」和『』。
...
const BOOKTRAVEL_ENTRY_PROMPT = `你是穿书入场导演。你负责基于已装配的世界模型，为用户设计进入小说世界的入口和可扮演身份。
...
## 输出格式
输出严格 JSON，字段为 entryPoints 与 recommendedUserCharacters。`
const BOOKTRAVEL_PLANNER_PROMPT = `你是穿书剧情规划师。你负责根据用户输入和当前穿书状态，规划新场景的场景信息、状态变化、剧情走向和场景写作指令。

## 输出要求
- 只输出 JSON，不要输出 Markdown 代码块，不要解释性文字。
- 所有 JSON 字符串值中不得包含未转义的 ASCII 双引号。如需引述，请使用中文引号「」和『』。

## 输出字段
id、title、summary（100字以内）、currentSituation、time、location、activeCharacters（数组）、stateChanges（对象）、divergence、storyProgress（0-100数字）、endingStatus（none/active/resolved/tragedy/open）、sceneGoals（数组）、entryBeatGuidance、writerInstructions。`
const BOOKTRAVEL_WRITER_PROMPT = `你是穿书场景写手。你负责根据剧情规划，写出沉浸感强、可游玩的中文场景和节拍。

## 自由输入架构（非常重要）
用户每回合自由输入行动文本，你每次被调用只负责写 1 个 beat，回应当前用户输入即可。不要预写后续步骤。
用户输入有三种前缀：【说话】是角色直接言语；【行为】是角色动作或试探；【剧情推进】是用户以旁白口吻提出的后续方向。你必须顺着输入类型展开剧情。

## 两种调用模式
1. insert-beat：在当前场景追加 1 个新 beat，不得改场景其他字段，beat 为单个对象。
2. change-scene：规划师已建新场景，你只输出 1 个入口节拍，不要返回场景元数据。

## 输出要求
- 只输出 JSON，不要输出 Markdown 代码块，不要解释性文字。
- 只输出 beat、volatileMemoryPatch 和 suggestedChoices 三个顶层字段。
- 叙事要有网文画面感，beat.content 不同段落用空行分隔。
- 所有 JSON 字符串值中不得包含未转义的 ASCII 双引号。如需引述，请使用中文引号「」和『』。
- beat 包含 id 与 content；volatileMemoryPatch 为对象（可空）；suggestedChoices 为 3 个后续走向字符串数组。`
const BOOKTRAVEL_MEMORY_PROMPT = `你是穿书记忆整理员。你负责把长线穿书游玩记录压缩成可继续使用的摘要记忆。

## 输出要求
- 只输出 JSON，不要输出 Markdown 代码块。
- 保留关键选择、状态变化、场景历史、重要互动、人物关系、已知秘密、未解决冲突和对原大纲的偏离。
- 不得删除仍然影响当前局势的重要信息。
- 输出字段：summary、keyChoices、unresolvedConflicts、divergenceFromOutline。`
const BOOKTRAVEL_ENDING_PROMPT = `你是穿书结局裁判。你负责根据当前穿书状态、剧情进度、用户关键选择，以及规划器给出的结局状态，判断故事是否进入结局，并生成最终总结。

## 输出要求
- 只输出 JSON，不要输出 Markdown 代码块，不要解释性文字。
- 所有面向用户的总结文字必须使用中文。
- 输出字段：finalEnding、userKeyChoices、originalOutlineComparison、characterOutcomes、worldlineName、divergenceScore（0-100）。`
// 穿书 JSON 解析：去代码块围栏 + 修复字符串内未转义控制字符（移植自原版 extract_json_text）。
function parseBooktravelJson(raw) {
  let text = String(raw || '').trim()
  if (text.startsWith('```')) {
    text = text.replace(/^```(?:json|JSON)?/, '').replace(/```$/, '').trim()
  }
...
// 穿书素材取文本：按 id 在指定资源类中找，返回正文/摘要/描述拼接。
function booktravelMaterialText(store, id, kinds) {
  if (!id) return ''
  const sid = String(id)
  for (const kind of kinds) {
    const rec = store.data.resources[kind] && store.data.resources[kind][sid]
    if (!rec) continue
    const d = rec.data || {}
    const parts = []
    if (d.text) parts.push(String(d.text))
    if (d.summary) parts.push(String(d.summary))
    // novels 存 chapters 数组
    if (Array.isArray(d.chapters)) parts.push(d.chapters.slice(0, 10).map(c => `第${c.index || ''}章 ${c.title || ''}\n${String(c.content || '').slice(0, 2000)}`).join('\n\n'))
    if (Array.isArray(d.characters)) parts.push(d.characters.slice(0, 10).map(c => typeof c === 'string' ? c : String((c && (c.name || c.description)) || '')).join('\n'))
    if (d.raw && typeof d.raw === 'object') {
      const r = d.raw
      if (r.description) parts.push(String(r.description))
      if (r.personality) parts.push(String(r.personality))
      if (r.scenario) parts.push(String(r.scenario))
      if (r.first_mes) parts.push(String(r.first_mes))
      if (Array.isArray(r.entries)) parts.push(r.entries.slice(0, 20).map(e => String((e && (e.content || e.text)) || '')).join('\n'))
    }
    if (d.normalized && typeof d.normalized === 'object') {
      const n = d.normalized
      if (n.description) parts.push(String(n.description))
      if (n.personality) parts.push(String(n.personality))
    }
    const text = parts.filter(Boolean).join('\n').trim()
    if (text) return rec.name ? `【${rec.name}】\n${text}` : text
  }
  return ''
}
      ...
      handler: async (req, res) => {
        ...
        try {
          ...
          if (req.method === 'POST' && pathname === '/plugins/creative-suite/import/card') {
            ...
            // PNG 角色卡：前端传 {image: dataUrl}，此处解出 card 后走同一 normalize 链。
            let cardBody = body
            if (!body.card && !body.text && typeof body.image === 'string' && body.image.trim() !== '') {
              const { card } = extractCardFromPngDataUrl(body.image.trim())
              cardBody = { ...body, card }
            }
            const { normalized, raw, name } = normalizeStCardInput(cardBody)
          ...
          if (req.method === 'POST' && pathname === '/plugins/creative-suite/pipeline/novel-to-card') {
            ...
            // Build a compact source: chapter outline + first/last slices per chapter,
            // so long novels don't blow up the LLM prompt or return non-JSON.
            const sourceText = (() => {
              const chapters = Array.isArray(novel.chapters) ? novel.chapters : []
              if (!chapters.length) return novel.text.slice(0, 12000)
              const parts = []
              // Global outline first (titles + previews).
              parts.push('章节目录：\n' + chapters.map(c => `第${c.index}章 ${c.title}（${c.charCount}字）：${String(c.preview || '').slice(0, 120)}`).join('\n'))
              for (const c of chapters) {
                const content = String(c.content || '')
                if (content.length <= 3000) {
                  parts.push(`\n--- 第${c.index}章 ${c.title} 全文 ---\n${content}`)
                } else {
                  parts.push(`\n--- 第${c.index}章 ${c.title} 开头 ---\n${content.slice(0, 1500)}\n--- 第${c.index}章 ${c.title} 结尾 ---\n${content.slice(-1500)}`)
                }
              }
              let out = parts.join('\n')
          ...
          if (req.method === 'POST' && pathname === '/plugins/creative-suite/museai/chat') {
            ...
            let result
            try {
              result = await generateTextWithRetry(routeCtx, {
          ...
          // N8 穿书模式（移植自 MuseAI 原版 book_travel.rs）：素材装配→入口→场景循环→记忆→结局。
          // 素材来自现有资源库（novels/worldbooks/cards），模型走 generateTextWithRetry。
          if (req.method === 'POST' && pathname === '/plugins/creative-suite/museai/booktravel/assemble') {
            const body = await readJsonBody(req, 4 * 1024 * 1024)
            const outlineText = booktravelMaterialText(store, body.outlineId, ['novels', 'stories', 'summaries'])
            const worldText = booktravelMaterialText(store, body.worldbookId, ['worldbooks'])
            const cardTexts = normalizeMuseIdList(body.characterCardIds || body.cardIds || []).map(id => booktravelMaterialText(store, id, ['cards'])).filter(Boolean)
            if (!outlineText && !worldText && cardTexts.length === 0) throw new Error('请至少选择一份大纲/世界书/角色卡素材')
            const userPrompt = `大纲：\n${outlineText.slice(0, 8000) || '（无）'}\n\n世界书：\n${worldText.slice(0, 8000) || '（无）'}\n\n角色卡：\n${cardTexts.map((t, i) => `角色${i + 1}：\n${t.slice(0, 3000)}`).join('\n\n')}`
            let result
            let parsed
            for (let attempt = 0; attempt < 2; attempt++) {
              try {
                result = await generateTextWithRetry(routeCtx, { system: BOOKTRAVEL_ASSEMBLER_PROMPT, prompt: userPrompt, maxTokens: 4000, temperature: 0 })
                parsed = parseBooktravelJson(result.text)
                break
              } catch (_e) {
                if (attempt === 1) throw _e
              }
            }
            sendJson(res, 200, { ok: true, ...parsed, outlineId: body.outlineId || null, worldbookId: body.worldbookId || null, characterCardIds: normalizeMuseIdList(body.characterCardIds || body.cardIds || []) })
            return
          }

          if (req.method === 'POST' && pathname === '/plugins/creative-suite/museai/booktravel/entry') {
            const body = await readJsonBody(req, 2 * 1024 * 1024)
            const worldModel = body.assembledWorldModel || body.worldModel || null
            if (!worldModel) throw new Error('assembledWorldModel 不能为空（请先调用 assemble）')
            const userPrompt = `已装配的世界模型：\n${JSON.stringify(worldModel).slice(0, 8000)}`
            let result
            let parsed
            for (let attempt = 0; attempt < 2; attempt++) {
              try {
                result = await generateTextWithRetry(routeCtx, { system: BOOKTRAVEL_ENTRY_PROMPT, prompt: userPrompt, maxTokens: 3000, temperature: 0 })
                parsed = parseBooktravelJson(result.text)
                break
              } catch (_e) {
                if (attempt === 1) throw _e
          ...
          if (req.method === 'POST' && pathname === '/plugins/creative-suite/museai/booktravel/turn') {
            const body = await readJsonBody(req, 4 * 1024 * 1024)
            const sessionId = String(body.sessionId || '').trim()
            if (!sessionId) throw new Error('sessionId 不能为空')
            const session = store.data.resources.sessions && store.data.resources.sessions[sessionId]
            if (!session) throw new Error('会话不存在')
            const bt = session.data.booktravelState || {}
            const userInput = String(body.userInput || body.message || '').trim()
            if (!userInput) throw new Error('userInput 不能为空')
            // 首轮 seed：body 可带 assembledWorldModel / entryPoint / userCharacter，直接初始化。
            if (body.assembledWorldModel && !bt.assembledWorldModel) bt.assembledWorldModel = body.assembledWorldModel
            if (body.entryPoint && !bt.entryPoint) bt.entryPoint = body.entryPoint
            if (body.userCharacter && !bt.userCharacter) bt.userCharacter = body.userCharacter
            const worldModelJson = JSON.stringify(bt.assembledWorldModel || {}).slice(0, 6000)
            const sceneJson = JSON.stringify(bt.currentScene || {}).slice(0, 3000)
            const memJson = JSON.stringify({ stable: bt.stableMemory || {}, volatile: bt.volatileMemory || {} }).slice(0, 2000)
            // 1) 剧情规划（首轮或换场景意图时全量规划，否则轻量沿用当前场景）
            const needPlan = !bt.currentScene || /换场景|转场|下一幕|新场景/.test(userInput)
            let plan = bt.currentPlan || null
            if (needPlan) {
              const planPrompt = `世界模型：${worldModelJson}\n\n当前记忆：${memJson}\n\n用户输入：${userInput}\n\n当前进度：${bt.storyProgress || 0}%`
              const planResult = await generateTextWithRetry(routeCtx, { system: BOOKTRAVEL_PLANNER_PROMPT, prompt: planPrompt, maxTokens: 2000, temperature: 0.2 })
              plan = parseBooktravelJson(planResult.text)
              bt.currentScene = { id: plan.id || randomUUID(), title: plan.title || '新场景', summary: plan.summary || '', currentSituation: plan.currentSituation || '', time: plan.time || '', location: plan.location || '', activeCharacters: plan.activeCharacters || [] }
              bt.currentPlan = plan
              bt.storyProgress = typeof plan.storyProgress === 'number' ? plan.storyProgress : (bt.storyProgress || 0)
            }
            // 2) 场景写作：每次只写 1 beat
            const writePrompt = `剧情规划：${JSON.stringify(plan || {}).slice(0, 3000)}\n\n当前场景：${sceneJson}\n\n用户输入：${userInput}\n\n调用模式：${needPlan ? 'change-scene' : 'insert-beat'}`
            const writeResult = await generateTextWithRetry(routeCtx, { system: BOOKTRAVEL_WRITER_PROMPT, prompt: writePrompt, maxTokens: 2000, temperature: 0.8 })
            const written = parseBooktravelJson(writeResult.text)
            const beat = written.beat || { id: randomUUID(), content: writeResult.text }
            // 3) 记忆合并 + 落会话
            if (written.volatileMemoryPatch && typeof written.volatileMemoryPatch === 'object') {
              bt.volatileMemory = { ...(bt.volatileMemory || {}), ...written.volatileMemoryPatch }
            }
            bt.turnHistory = [...(bt.turnHistory || []), { userInput, beatId: beat.id, at: Date.now() }]
            const now = Date.now()
            session.data.booktravelState = bt
            session.data.messages = [...(session.data.messages || []), { role: 'user', content: userInput, at: now }, { role: 'assistant', content: beat.content || '', at: now }]
            session.updatedAt = now
            await store.save()
            // 4) 结局判定（进度>=95 或规划器 endingStatus 非 none 时触发）
            let ending = null
            const endingStatus = plan && plan.endingStatus
            if ((bt.storyProgress || 0) >= 95 || (endingStatus && endingStatus !== 'none')) {
              try {
                const endPrompt = `世界模型：${worldModelJson}\n\n当前进度：${bt.storyProgress || 0}%\n\n规划器结局状态：${endingStatus || 'none'}\n\n关键选择：${JSON.stringify((bt.turnHistory || []).slice(-10))}`
                const endResult = await generateTextWithRetry(routeCtx, { system: BOOKTRAVEL_ENDING_PROMPT, prompt: endPrompt, maxTokens: 2000, temperature: 0.3 })
                const endParsed = parseBooktravelJson(endResult.text)
                if (endParsed.finalEnding) { ending = endParsed; bt.ending = endParsed }
                session.data.booktravelState = bt
                await store.save()
              } catch (_e) {}
            }
            sendJson(res, 200, { ok: true, beat, volatileMemoryPatch: written.volatileMemoryPatch || {}, suggestedChoices: written.suggestedChoices || [], storyProgress: bt.storyProgress || 0, currentScene: bt.currentScene, ending })
            return
          }

          if (req.method === 'POST' && pathname === '/plugins/creative-suite/museai/booktravel/memory') {
            const body = await readJsonBody(req)
            const sessionId = String(body.sessionId || '').trim()
            if (!sessionId) throw new Error('sessionId 不能为空')
            const session = store.data.resources.sessions && store.data.resources.sessions[sessionId]
            if (!session) throw new Error('会话不存在')
            const bt = session.data.booktravelState || {}
            const historyText = (session.data.messages || []).slice(-30).map(m => `${m.role}：${String(m.content || '').slice(0, 500)}`).join('\n')
            const result = await generateTextWithRetry(routeCtx, { system: BOOKTRAVEL_MEMORY_PROMPT, prompt: `游玩记录：\n${historyText}`, maxTokens: 2000, temperature: 0 })
            const parsed = parseBooktravelJson(result.text)
            bt.stableMemory = { ...(bt.stableMemory || {}), lastSummary: parsed.summary || '' }
            session.data.booktravelState = bt
            await store.save()
            sendJson(res, 200, { ok: true, ...parsed })
            return
          ...
          if (req.method === 'GET' && pathname === '/plugins/creative-suite/tavern/gameplay/cards') {
            // N6: 本地化 cards —— 列出 store 内 cards（id/name）。
            const list = Object.values(store.data.resources.cards || {}).map(c => ({ id: c.id, name: c.name }))
            sendJson(res, 200, { ok: true, native: true, cards: list })
            return
          }

          if (req.method === 'GET' && pathname === '/plugins/creative-suite/tavern/gameplay/state') {
            // N6: 本地化 state —— 返回 helper runtime snapshot。
            const query = new URL(req.url || '/', 'http://x').searchParams
            const sessionId = String(query.get('sessionId') || '').trim()
            if (!sessionId) { sendJson(res, 400, { ok: false, error: 'sessionId 不能为空' }); return }
            const runtime = getHelperRuntime(sessionId)
            if (!runtime) { sendJson(res, 404, { ok: false, error: 'helper runtime not found' }); return }
            sendJson(res, 200, { ok: true, native: true, runtime: runtime.snapshot() })
            return
          }

          if (req.method === 'POST' && pathname === '/plugins/creative-suite/tavern/gameplay/create') {
            // N6: 本地化 create —— 建 helper runtime 并返回问候语。
            const body = await readJsonBody(req, 2 * 1024 * 1024)
            const cardId = String(body.cardId || body.sourceCard || '').trim()
            if (!cardId) { sendJson(res, 400, { ok: false, error: 'cardId 不能为空' }); return }
            const card = store.data.resources.cards && (store.data.resources.cards[cardId] || Object.values(store.data.resources.cards).find(c => c.name === cardId))
            if (!card) { sendJson(res, 404, { ok: false, error: 'card not found' }); return }
            const raw = card.data && card.data.raw
            // N7: create 收 presetId（可选），校验存在后存 runtime。
            const createPresetId = String(body.presetId || '').trim()
            if (createPresetId && !(store.data.resources.presets && store.data.resources.presets[createPresetId])) { sendJson(res, 404, { ok: false, error: 'preset not found' }); return }
            const runtime = createHelperRuntime({ cardId: card.id, rawCard: raw, variables: {}, messages: [], sessionId: '', cardName: card.name || '', userName: String(body.userName || '你'), extensionSettings: createPresetId ? { presetId: createPresetId } : {} })
            let greeting = '你好，我是' + (card.name || '此角色') + '。'
            try {
              const unwrapped = raw && raw.data ? raw : null
              const first = unwrapped && (unwrapped.first_mes || (unwrapped.data && unwrapped.data.first_mes))
              if (first) greeting = String(first)
            } catch (_e) {}
            sendJson(res, 200, { ok: true, native: true, runtimeId: runtime.id, sessionId: runtime.id, greeting })
            return
          }

          if (req.method === 'POST' && pathname === '/plugins/creative-suite/tavern/gameplay/send') {
            // N6: 本地化 send —— user消息落runtime，经 native/turn 同款链路生成回复。
            const body = await readJsonBody(req, 2 * 1024 * 1024)
            const sessionId = String(body.sessionId || body.runtimeId || '').trim()
            if (!sessionId) { sendJson(res, 400, { ok: false, error: 'sessionId 不能为空' }); return }
            const runtime = getHelperRuntime(sessionId)
            if (!runtime) { sendJson(res, 404, { ok: false, error: 'helper runtime not found' }); return }
            const input = String(body.input ?? body.message ?? '')
            if (!input.trim()) { sendJson(res, 400, { ok: false, error: 'input 不能为空' }); return }
            const userMsg = runtime.createMessage({ role: 'user', content: input.trim() })
            await runtime.messageReceived({ message: userMsg }).catch(() => {})
            let generated
            try {
              // N7: presetId 随 send 覆盖或沿用 runtime 绑定；prompts 进 system，输出过 preset regex。
              const sendPresetId = String(body.presetId || (runtime.extensionSettings && runtime.extensionSettings.presetId) || '').trim()
              if (body.presetId !== undefined) runtime.extensionSettings = { ...(runtime.extensionSettings || {}), presetId: sendPresetId }
              const museCtx = buildMuseContext(store, { cardId: runtime.cardId, presetId: sendPresetId })
              generated = await generateTextWithRetry(routeCtx, { system: buildMuseSystem(museCtx, 'tavern', {}), prompt: input.trim(), maxTokens: 2000 })
              if (museCtx.presetRecord) {
                try {
                  const filtered = applyRegexScripts(extractRegexScriptsFromPreset(museCtx.presetRecord), generated.text, { forDisplay: true })
                  generated = { ...generated, text: filtered.text }
                } catch (_e) {}
              }
            } catch (error) {
            ...
            const assistantMsg = runtime.createMessage({ role: 'assistant', content: generated.text })
            await runtime.messageSent({ message: assistantMsg }).catch(() => {})
            sendJson(res, 200, { ok: true, native: true, userMessage: userMsg, assistantMessage: assistantMsg, model: generated.model })
            return
          }

          if (req.method === 'POST' && pathname === '/plugins/creative-suite/tavern/gameplay/candidates') {
            // N6: 本地化 candidates —— 经 runtime.createCandidates 建槽位。
            const body = await readJsonBody(req, 2 * 1024 * 1024)
            const sessionId = String(body.sessionId || body.runtimeId || '').trim()
            if (!sessionId) { sendJson(res, 400, { ok: false, error: 'sessionId 不能为空' }); return }
            const runtime = getHelperRuntime(sessionId)
            if (!runtime) { sendJson(res, 404, { ok: false, error: 'helper runtime not found' }); return }
            const out = runtime.createCandidates(body.count || 3, { label: String(body.label || '') })
            sendJson(res, 200, { ok: true, native: true, batchId: out.batchId, items: out.items })
            return
          }

          if (req.method === 'POST' && pathname === '/plugins/creative-suite/tavern/gameplay/variables') {
            // N6: 本地化 variables —— 写 runtime.variables。
            const body = await readJsonBody(req, 4 * 1024 * 1024)
            const sessionId = String(body.sessionId || body.runtimeId || '').trim()
            if (!sessionId) { sendJson(res, 400, { ok: false, error: 'sessionId 不能为空' }); return }
            const runtime = getHelperRuntime(sessionId)
            ...
            sendJson(res, 200, { ok: true, native: true, variables: runtime.getVariable() })
            return
          }

          if (req.method === 'POST' && pathname === '/plugins/creative-suite/tavern/gameplay/cancel') {
            // N6: 本地化 cancel —— 停止 runtime。
            const body = await readJsonBody(req, 2 * 1024 * 1024)
            const sessionId = String(body.sessionId || body.runtimeId || '').trim()
            if (!sessionId) { sendJson(res, 400, { ok: false, error: 'sessionId 不能为空' }); return }
            const runtime = getHelperRuntime(sessionId)
            if (!runtime) { sendJson(res, 404, { ok: false, error: 'helper runtime not found' }); return }
            const snapped = runtime.stop(String(body.reason || 'cancelled'))
            sendJson(res, 200, { ok: true, native: true, runtime: snapped })
            return
          }
          ...
          if (req.method === 'POST' && pathname === '/plugins/creative-suite/tavern/chat/regen') {
            // N6: 本地化 regen —— 经 helper runtime 去掉最后一条 assistant 消息。
            // 入参 { runtimeId } 或 { sessionId }（sessionId 复用同 id 的 runtime）。
            const body = await readJsonBody(req, 2 * 1024 * 1024)
            const key = String(body.runtimeId || body.sessionId || '').trim()
            if (!key) { sendJson(res, 400, { ok: false, error: 'runtimeId 或 sessionId 不能为空' }); return }
            const runtime = getHelperRuntime(key)
            if (!runtime) { sendJson(res, 404, { ok: false, error: 'helper runtime not found' }); return }
            const out = runtime.regenLast()
            sendJson(res, 200, { ok: true, native: true, removed: out.removed, messages: out.messages })
            return
          }

          if (req.method === 'POST' && pathname === '/plugins/creative-suite/tavern/chat/rollback') {
            // N6: 本地化 rollback —— 入参 { runtimeId|sessionId, index }。
            const body = await readJsonBody(req, 2 * 1024 * 1024)
            const key = String(body.runtimeId || body.sessionId || '').trim()
            if (!key) { sendJson(res, 400, { ok: false, error: 'runtimeId 或 sessionId 不能为空' }); return }
            const runtime = getHelperRuntime(key)
            if (!runtime) { sendJson(res, 404, { ok: false, error: 'helper runtime not found' }); return }
            const index = Number(body.index ?? body.at ?? -1)
            if (!Number.isFinite(index)) { sendJson(res, 400, { ok: false, error: 'index 必须是数字' }); return }
            const out = runtime.rollbackTo(index, { label: String(body.label || '') })
            sendJson(res, 200, { ok: true, native: true, at: out.at, kept: out.kept, removed: out.removed })
            return
          }

          if (req.method === 'POST' && pathname === '/plugins/creative-suite/tavern/chat/undo-rollback') {
            // N6: 本地化 undo-rollback —— 入参 { runtimeId|sessionId }。
            const body = await readJsonBody(req, 2 * 1024 * 1024)
            const key = String(body.runtimeId || body.sessionId || '').trim()
            if (!key) { sendJson(res, 400, { ok: false, error: 'runtimeId 或 sessionId 不能为空' }); return }
            const runtime = getHelperRuntime(key)
            if (!runtime) { sendJson(res, 404, { ok: false, error: 'helper runtime not found' }); return }
            const out = runtime.undoRollback()
            if (!out) { sendJson(res, 409, { ok: false, error: '没有可撤销的回滚' }); return }
            sendJson(res, 200, { ok: true, native: true, label: out.label, messages: out.messages, variables: out.variables })
            return
          }

          if (req.method === 'POST' && pathname === '/plugins/creative-suite/tavern/chat/retry-mvu') {
            // N6: 本地化 retry-mvu —— 入参 { runtimeId|sessionId, settlementId?, turn?, label? }。
            // 语义：对 pending/failed 超时的 settlement 重开一轮（beginSettlement），旧的不动。
            const body = await readJsonBody(req, 2 * 1024 * 1024)
            const key = String(body.runtimeId || body.sessionId || '').trim()
            if (!key) { sendJson(res, 400, { ok: false, error: 'runtimeId 或 sessionId 不能为空' }); return }
            const runtime = getHelperRuntime(key)
            if (!runtime) { sendJson(res, 404, { ok: false, error: 'helper runtime not found' }); return }
            const settlement = runtime.beginSettlement({ turn: body.turn, label: String(body.label || 'retry') })
            sendJson(res, 200, { ok: true, native: true, settlement })
            return
          }
