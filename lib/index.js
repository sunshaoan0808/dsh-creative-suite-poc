        ...
        try {
          ...
          if (req.method === 'POST' && pathname === '/plugins/creative-suite/museai/booktravel/assemble') {
            ...
            for (let attempt = 0; attempt < 2; attempt++) {
              try {
                result = await generateTextWithRetry(routeCtx, { system: BOOKTRAVEL_ASSEMBLER_PROMPT, prompt: userPrompt, maxTokens: 6000, temperature: 0 })
          ...
          if (req.method === 'POST' && pathname === '/plugins/creative-suite/museai/booktravel/entry') {
            ...
            for (let attempt = 0; attempt < 4; attempt++) {
              try {
                result = await generateTextWithRetry(routeCtx, { system: BOOKTRAVEL_ENTRY_PROMPT, prompt: userPrompt, maxTokens: 6000, temperature: 0 })
                parsed = parseBooktravelJson(result.text)
                break
              } catch (_e) {
