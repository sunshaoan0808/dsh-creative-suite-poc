  ...
  factory: (require) => {
    ...
    function CreativeSuitePoc() {
      ...
      const resourcePager = react.createElement('div', { style: { display: 'flex', gap: '5px', flexWrap: 'wrap', alignItems: 'center', margin: '6px 0', fontSize: '11px' } },
        react.createElement('select', {
          ...
          style: { padding: '4px 6px', borderRadius: '6px', border: '1px solid rgba(255,255,255,.18)', background: 'rgba(0,0,0,.25)', color: 'inherit', fontSize: '11px' }
        },
        ...
        react.createElement('input', {
          ...
          placeholder: '跳页',
          style: { width: '56px', padding: '4px 6px', borderRadius: '6px', border: '1px solid rgba(255,255,255,.18)', background: 'rgba(0,0,0,.25)', color: 'inherit', fontSize: '11px' }
        }),
        react.createElement('button', {
          type: 'button', className: 'dsh-tavern-entry-btn',
          style: { width: 'auto', margin: 0, padding: '4px 8px', fontSize: '11px' },
    ...
    function MuseAIPane() {
      ...
      react.useEffect(() => {
        ...
        ]).then(([cardPayload, worldbookPayload, sessionPayload, storyPayload]) => {
          if (stopped) return
    ...
    function TavernDiagPane() {
      ...
      async function refresh() {
        ...
        try {
          ...
          const next = defs.map((def, index) => {
            const entry = settled[index]
            if (entry.status !== 'fulfilled') return { label: def.label, ok: false, text: String(entry.reason && entry.reason.message || entry.reason) }
          ...
          setVerdict(failed.length === 0 ? '✅ 全部就绪' : '⚠️ ' + failed.length + ' 项异常：' + failed.map(item => item.label).join('、'))
          const dump = {}
          defs.forEach((def, index) => { dump[def.key] = settled[index].status === 'fulfilled' ? settled[index].value : { error: String(settled[index].reason && settled[index].reason.message || settled[index].reason) } })
          setRaw(JSON.stringify(dump, null, 2).slice(0, 8000))
        } catch (err) {
          setError(String(err && err.message || err))
        } finally {
          setBusy(false)
    ...
    function TavernModePane(props) {
      ...
      return react.createElement(react.Fragment, null,
        ...
        tab === 'play' ? react.createElement(TavernPlayPane, props || {}) : react.createElement(TavernBridgePane)
      )
    }
    ...
    function CreativeWorkspaceView() {
      ...
      const workspaceModes = [
        ...
        { id: 'cards', label: '卡片工作台' },
        { id: 'regex', label: '正则预览' },
        { id: 'story', label: 'Story' },
        { id: 'museai', label: 'MuseAI' },
        { id: 'tavern', label: '酒馆桥接' }
      ]
    ...
    function apply(ctx) {
      ctx.effect(() => ctx.slots.inject('sidebar.footer.action', () => ctx.slots.register({
        ...
        id: 'dsh-creative-suite-poc',
        priority: 998
      }, CreativeSuitePoc)), 'creative-suite: sidebar POC entry')
      ctx.effect(() => ctx.slots.inject('conversation.view', () => ctx.slots.register({
        name: 'conversation.view',
        id: 'creative-suite',
        order: 15,
        label: () => '创作'
      }, CreativeWorkspaceView)), 'creative-suite: conversation view')
    }
