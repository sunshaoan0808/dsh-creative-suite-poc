  ...
  factory: (require) => {
    ...
    function MuseAIPane() {
      ...
      if (page === 'background') {
        ...
        const [btWorldbookId, setBtWorldbookId] = react.useState(worldbookId || '')
        ...
        react.useEffect(() => {
          ...
          requestJson('/plugins/creative-suite/resources/novels')
            .then(payload => { if (!stopped) setBtNovels(payload.resources || []) })
            .catch(() => {})
          return () => { stopped = true }
        }, [refreshKey])
        ...
        if (btPhase === 'materials') {
          ...
          body = react.createElement(react.Fragment, null,
            ...
            react.createElement('div', { style: { height: '6px', borderRadius: '3px', background: 'rgba(255,255,255,.1)', marginBottom: '10px' } },
              react.createElement('div', { style: { width: `${Math.min(100, btProgress)}%`, height: '100%', borderRadius: '3px', background: 'linear-gradient(90deg,#7db4ff,#b48cff)' } })
            ),
