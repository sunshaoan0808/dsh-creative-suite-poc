...
日期：2026-09-25。POC 版本：`dsh-creative-suite-poc@0.17.1`。POC 测试：39/39 绿（已推送）。
...
| `dsh-creative-suite-poc` | **0.17.1** | `GET /plugins/creative-suite/status` 返回 `version: '0.17.1'`（handler 硬编码，须与 host 安装 bundle 版本交叉核对，不盲信） | POC 侧完成，待 host 交叉核对机制 |
...
- POC 声明：`package.json:2-3` version 0.17.1；`peerDependencies` cordis `^4.0.1` / react `^18.2.0`。
...
3. 定 `GET /status` 硬编码 `version: '0.17.1'` 与 host 安装 bundle 版本的交叉核对机制。
